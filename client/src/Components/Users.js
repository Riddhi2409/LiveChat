...
function Users({ darkMode }) {
  ...
  const fecthUsers = async () => {
    ...
    await axios
      .get("http://localhost:8080/auth/fetchuser", config)
      .then((data) => {
        ...
        {users.map((userr, index) => {
          return (
            ...
            whileTap={{ scale: 0.98 }}
              ...
              key={userr._id}
              onClick={() => {
                // console.log("Creating chat with ", userr.name);
                const config = {
                  headers: {
                    Authorization: `Bearer ${user}`,
                  },
                 ...
                 axios.post(
                  "http://localhost:8080/chat/access",
                  {
                    userId: userr._id,
                  },
                  config
            ...
            >
              ...
              <p className="font-bold text-lg text-slate-500">{userr.name}</p>
            </motion.div>
          );
        })} 
      </div>
